...
// Available products to choose from randomly (matches seed.ts)
const AVAILABLE_PRODUCTS = [
  'Performance Profiler Pro',
  'Security Scanner Elite',
  'Unit Test Generator',
  'Code Coverage Reporter',
  'API Mocking Tool',
  'Dependency Checker Pro',
...
test.describe('E-commerce Load Test Configuration', () => {
  test.beforeAll(async () => {
    ...
    console.log(`   • Available products: ${AVAILABLE_PRODUCTS.length}`);
    console.log(`   • Target URL: baseURL from config\n`);
  });
});
...
test('Performance Benchmark', async ({ page }) => {
  ...
  for (let i = 1; i <= benchmarkRuns; i++) {
    ...
    await loginUser(page);
    await addProductToCart(page, 'API Mocking Tool'); // Use consistent product for benchmarking
    await page.getByTestId('header-cart-button').click();
    await page.getByTestId('checkout-button').click();
    await expect(page.getByText('Order placed successfully!')).toBeVisible();
    await page.getByRole('link', { name: 'Continue Shopping' }).click();
