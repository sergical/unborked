...
router.post('/borkedpay', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
        ...
        console.error('Checkout error caught:', errorMessage, err?.stack);
        return res.status(500).json({ error: errorMessage });
    }
});
// Mock payment processing function that simulates real gateway behavior
async function processPayment(request, config) {
