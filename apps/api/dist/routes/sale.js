...
router.get('/', async (_req, res) => {
    try {
        ...
        for (const product of allProducts) {
            ...
            const salePrice = await db_1.db
                .select()
                .from(schema_1.salePrices)
                .where((0, drizzle_orm_1.eq)(schema_1.salePrices.productId, product.id))
                .limit(1);
            if (salePrice.length > 0) {
                // Another individual query for metadata (double N+1!)
                const metadata = await db_1.db
                    .select()
                    .from(schema_1.productMetadata)
                    .where((0, drizzle_orm_1.eq)(schema_1.productMetadata.productId, product.id))
                    .limit(1);
                ...
                if (metadata.length > 0 && metadata[0].saleCategory) {
                    const categoryResult = await db_1.db
                        .select()
                        .from(schema_1.saleCategories)
                        .where((0, drizzle_orm_1.eq)(schema_1.saleCategories.name, metadata[0].saleCategory))
                        .limit(1);
