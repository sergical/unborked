...
async function seedProducts() {
    ...
    for (const product of baseProducts) {
        ...
        const [newProduct] = await index_1.db.insert(schema_1.products).values({
            name: product.name,
...
async function seedSaleData(insertedProducts) {
    ...
    for (const sale of saleMetadata) {
        ...
        for (const product of matchingProducts) {
            ...
            await index_1.db.insert(schema_1.salePrices).values({
            ...
            await index_1.db.insert(schema_1.productMetadata).values({
                productId: product.id,
                discount: discount,
...
async function seed() {
    try {
        ...
        await db_config_1.default.query('TRUNCATE TABLE user_carts, purchases, product_metadata, sale_prices, products, sale_categories, users RESTART IDENTITY CASCADE');
        console.log('Database cleared.');
        // Create sale categories
        console.log('Creating sale categories...');
        await index_1.db.insert(schema_1.saleCategories).values([
            {
                name: 'Black Friday',
